...
func newTwoFATestServer(t *testing.T) (*httptest.Server, *store.Queries, *crypto.SecretBox, *http.Client, store.User) {
	...
	authH.SetTwoFA(twoH)
	srv := httptest.NewServer(NewRouter(logger, db, q, nil, nil, sessionSvc, authH, nil, nil, nil, nil, nil, nil, nil, nil, usersH, nil, nil, nil, twoH, nil, nil, false, "dev", "unknown"))
	t.Cleanup(srv.Close)
